...
export class ReAuthGuard implements CanActivate {
  ...
  canActivate(
    ...
    let user = localStorage.getItem('loggedIn');
     console.log({user});
      if (user) {
        this.api.admin ? this.router.navigate(['/home'], { skipLocationChange: true }) : this.router.navigate(['/tabs/chats/'], { queryParams: { name: 'Messenger', id: user }, skipLocationChange: false });
